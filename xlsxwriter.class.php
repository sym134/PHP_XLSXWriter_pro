...
class XLSXWriter
{
    ...
    public function __construct($data)
    {
        ...
        foreach ($data['sheet'] as $key => $sheet) {
            # 初始表
            $this->initializeSheet($key, $sheet['col_style']['widths'], $sheet['col_style']['auto_filter'], $sheet['col_style']['freeze_rows'], $sheet['col_style']['freeze_columns']);

            if (is_array($sheet['table_title'])) {
                # 初始表 标题
                $this->markMergedCell($key, 0, 0, 0, $sheet['table_title']['length']);
                $this->writeSheetRow($key, $sheet['table_title']['title'], $sheet['table_title']['style']);
            }

            # 初始表header
            if (is_array($sheet['header'])) {
                $this->writeSheetHeader($key, $sheet['header']['col_type'], $sheet['header']['style']);
            }

            # 写入表数据
            if (is_array($sheet['rows']['data'])) {
                foreach ($sheet['rows']['data'] as $row) {
                    $this->writeSheetRow($key, $row, $sheet['rows']['style']);
                }
            }
        }
        ...
        # 保存指定文件
        if ($data['is_save'])
            $this->writeToFile($data['filename'] . '.xlsx');
    }
     ...
     * @param array  $col_widths  单元格宽度
     * @param bool   $auto_filter 自动筛选
    ...
    protected function initializeSheet($sheet_name, $col_widths = array(), $auto_filter = false, $freeze_rows = false, $freeze_columns = false)
    {
        ...
        $sheet->file_writer->write('<sheetView tabSelected="1" workbookViewId="0">');
     ...
     * @param array  $col_options
     * @param int    $row_number 写入第几行
     *                           Date: 2019-05-07
     *                           Time: 16:04
     *                           Author: sym
     */
    public function writeSheetHeader($sheet_name, array $header_types, $col_options = null, $row_number = 1)
    {
        # 抑制行
        ...
        if (!$suppress_row) {
            ...
            foreach ($header_row as $c => $v) {
                ...
                $this->writeCell($sheet->file_writer, $row_number, $c, $v, $number_format_type = 'n_string', $cell_style_idx);
            }
    ...
    public function markMergedCell($sheet_name, $start_cell_row, $start_cell_column, $end_cell_row, $end_cell_column)
    {
        ...
        $this->sheets[$sheet_name]->merge_cells[] = $startCell . ":" . $endCell;
    }
     ...
     * @param array  $data
     * @param string $sheet_name
     * @param array  $header_types 标题配置，存在写入表头
     *                             Date: 2019-05-07
     *                             Time: 15:43
     *                             Author: sym
     */
